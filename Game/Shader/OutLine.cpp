...
/// インプットレイアウト
/// </summary>
const std::vector<D3D11_INPUT_ELEMENT_DESC> OutLine::INPUT_LAYOUT =
{
...
void OutLine::Update(float elapsedTime)
{
...
void OutLine::Render(ID3D11ShaderResourceView* depthTexture, ID3D11ShaderResourceView* normalTexture)
{
	auto context = Graphics::GetInstance()->GetDeviceResources()->GetD3DDeviceContext();
...
void OutLine::LoadShader(ID3D11Device* device)
{
	...
	// ライト用のピクセルシェーダをロードする
	BinaryFile ps = BinaryFile::LoadFile(L"Resources/Shaders/OutLinePS.cso");
	DX::ThrowIfFailed(
		device->CreatePixelShader(ps.GetData(), ps.GetSize(), nullptr, m_PS.ReleaseAndGetAddressOf())
	);

	// ライト用のピクセルシェーダをロードする
	BinaryFile vs = BinaryFile::LoadFile(L"Resources/Shaders/OutLineVS.cso");
	DX::ThrowIfFailed(
